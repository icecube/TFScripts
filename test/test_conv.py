...
class TestConvModule(unittest.TestCase):
    ...
    def test_locally_connected_2d(self):
        ...
        data = tf.constant(
            self.random_state.normal(size=[3, 2, 4, 2]), dtype=tf.float32
        )
        kernel = tf.constant(
            self.random_state.normal(size=[8, 18, 5]), dtype=tf.float32
        )
        num_outputs = 5
        filter_size = [3, 3]
        ...
        conv_layer = conv.LocallyConnected2d(
    ...
    def test_locally_connected_3d(self):
        ...
        data = tf.constant(
            self.random_state.normal(size=[1, 3, 3, 3, 2]), dtype=tf.float32
        )
        kernel = tf.constant(
            self.random_state.normal(size=[27, 54, 2]), dtype=tf.float32
        )
        num_outputs = 2
        filter_size = [3, 3, 3]
        ...
        conv_layer = conv.LocallyConnected3d(
            input_shape=data.get_shape(),
            num_outputs=num_outputs,
            filter_size=filter_size,
            kernel=kernel,
    ...
    def test_conv3d_stacked(self):
        """Test locally_connected_3d"""
        data = tf.constant(
            self.random_state.normal(size=[7, 5, 4, 7, 2]), dtype=tf.float32
        )
        kernel = tf.constant(
            self.random_state.normal(size=[3, 3, 3, 2, 4]), dtype=tf.float32
        )
        ...
        for strides, padding in product(strides_list, padding_list):
            ...
            result_tf = tf.nn.convolution(
                input=data,
                filters=kernel,
                strides=strides[1:-1],
                padding=padding,
            )
            result = conv.conv3d_stacked(
                input=data, filter=kernel, strides=strides, padding=padding
            )
    ...
    def test_conv4d_stacked(self):
        """Test locally_connected_3d"""
        data = tf.constant(
            self.random_state.normal(size=[1, 2, 2, 2, 2, 1]), dtype=tf.float32
        )
        kernel = tf.constant(
            self.random_state.normal(size=[2, 2, 2, 2, 1, 1]), dtype=tf.float32
        )

        true_result = [
            [
                [[[-3.4794035], [0.19396555]], [[3.3645966], [0.17348471]]],
                [[[-0.9240602], [1.229038]], [[-0.48166633], [-0.47118214]]],
            ],
            [
                [[[3.195785], [-2.968795]], [[-2.0781631], [-0.35241044]]],
                [[[1.5140774], [2.4484003]], [[1.5703532], [0.5695023]]],
            ],
        ]
