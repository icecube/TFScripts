...
class TestConvModule(unittest.TestCase):
    ...
    def test_ConvNdLayers(self):
        """Test ConvNdLayers"""
        data = tf.constant(
            self.random_state.normal(size=[3, 4, 4, 4, 2]), dtype=tf.float32
        )

        shapes = [[3, 3, 3, 2, 3], [3, 3, 3, 3, 7], [3, 3, 3, 7, 1]]
        weights_list = [
            tf.constant(self.random_state.normal(size=shape), dtype=tf.float32)
            for shape in shapes
        ]
    ...
    def test_FCLayers(self):
        """Test FCLayers"""
        data = tf.constant(
            self.random_state.normal(size=[3, 7]), dtype=tf.float32
        )
        ...
        result_true = [[0.06657974], [-0.3617597], [0.00241985]]
