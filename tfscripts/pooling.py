    ...
    if ksize[1:-1] != [1, 1, 1] or strides[1:-1] != [1, 1, 1]:
        if pooling_type == "max":
            layer = tf.nn.max_pool3d(
                input=layer, ksize=ksize, strides=strides, padding=padding
            )
        elif pooling_type == "avg":
            layer = tf.nn.avg_pool3d(
                input=layer, ksize=ksize, strides=strides, padding=padding
            )

        elif pooling_type == "max_avg":
            layer_max = tf.nn.max_pool3d(
                input=layer, ksize=ksize, strides=strides, padding=padding
            )
            layer_avg = tf.nn.avg_pool3d(
                input=layer, ksize=ksize, strides=strides, padding=padding
            )
            layer = (layer_avg + layer_max) / 2.0
...
def pool2d(layer, ksize, strides, padding, pooling_type):
    ...
    if pooling_type == "max":
        ...
        layer = tf.nn.avg_pool2d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
    elif pooling_type == "max_avg":
        layer_max = tf.nn.max_pool2d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
        layer_avg = tf.nn.avg_pool2d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
        layer = (layer_avg + layer_max) / 2.0
...
def pool1d(layer, ksize, strides, padding, pooling_type):
    ...
    # Use pooling to down-sample the image resolution?
    if pooling_type == "max":
        layer = tf.nn.max_pool1d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
    elif pooling_type == "avg":
        layer = tf.nn.avg_pool1d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
    elif pooling_type == "max_avg":
        layer_max = tf.nn.max_pool1d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
        layer_avg = tf.nn.avg_pool1d(
            input=layer, ksize=ksize, strides=strides, padding=padding
        )
        layer = (layer_avg + layer_max) / 2.0
...
def pool_over_depth(layer, ksize, stride, padding, pooling_type):
    ...
    for index in range(start_index, num_channels, stride):
        ...
        slice_c, padding_c = get_conv_slice(
            index,
            input_length=num_channels,
            filter_size=ksize,
            stride=stride,
        ...
        if pooling_type == "max":
            input_patches.append(
                tf.reduce_max(input_tensor=layer[..., slice_c], axis=-1)
            )

        elif pooling_type == "avg":
            input_patches.append(
                tf.reduce_mean(input_tensor=layer[..., slice_c], axis=-1)
            )

        else:
            raise ValueError(
                "Pooling_type {!r} is unknown.".format(pooling_type)
            )
...
def max_pool4d_stacked(input, ksize, strides, padding):
    ...
    if ksize[4] != 1:
        raise ValueError(
            "max_pool4d_stacked does not yet support "
...
def avg_pool4d_stacked(input, ksize, strides, padding):
    ...
    for i in range(start_index, size_of_t_dim, strides[4]):
        ...
        if padding == "VALID":
            # Get indices z_s
            indices_t_s = range(
                max(i - filter_l, 0), min(i + filter_r + 1, size_of_t_dim)
            )
            ...
            avg_tensors_t_s = tf.divide(tf.add_n(tensors_t_averaged), len_ts)
