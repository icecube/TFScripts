...
class AddResidual(tf.keras.layers.Layer):
    ...
    def call(self, input, residual):
        ...
        if self.use_scale_factor:
            ...
            if num_inputs == num_outputs:
                output = (residual + input) / np.sqrt(2.0)
            elif num_inputs > num_outputs:
                output = (residual + input[..., :num_outputs]) / np.sqrt(2.0)
            elif num_inputs < num_outputs:
                output = tf.concat(
                    [
                        (residual[..., :num_inputs] + input) / np.sqrt(2.0),
                        residual[..., num_inputs:],
                    ],
                    axis=-1,
                )
...
class Activation(tf.keras.layers.Layer):
    ...
    separate parametrizations should be learned.
    ...
    def call(self, layer, is_training=None):
        ...
        if hasattr(tf.nn, activation_type):
            ...
            layer = tf.multiply(
                tf.maximum(-0.01 * layer, layer), tf.sign(layer)
            )
        # todo: NecroRelu
        ...
        #    blob/master/nn_classifier.py
            ...
            layer = tf.where(
                tf.less(layer, 0.0), tf.exp(layer) * alpha - alpha, layer
            )
            layer = layer * lam

        elif activation_type == "centeredRelu":
            layer = tf.nn.relu6(layer) - 3.0

        elif activation_type == "negrelu":
            layer = -tf.nn.relu(layer)

        elif activation_type == "invrelu":
            layer = tf.where(tf.less(layer, 0.0), layer, (layer + 1e-8) ** -1)

        elif activation_type == "sign":
            layer = tf.where(tf.less(layer, 0.0), layer, tf.sign(layer))

        elif activation_type == "prelu":
            slope = self.slope_weight + 1.0
            layer = tf.where(tf.less(layer, 0.0), layer * slope, layer)
            ...
            layer = tf.exp(tf.square((layer - self.mu) / sigma) * (-0.5)) / (
                sigma
            )
    ...
    def call(self, inputs, is_training):
        ...
        if is_training:
            batch_mean, batch_var = tf.nn.moments(
                x=inputs, axes=[0], keepdims=False
            )
...
def maxout(inputs, num_units, axis=-1):
    ...
    if not isinstance(num_channels, tf.Tensor) and num_channels % num_units:
        raise ValueError(
