...
def activation(
    ...
    if use_batch_normalisation:
        ...
        if is_training is None:
            raise ValueError(
                "To use batch normalisation a boolean is_training"
                " needs to be passed"
            )
    ...
    #    blob/master/nn_classifier.py
        ...
        layer = tf.where(
            tf.less(layer, tf.constant(0, dtype=FLOAT_PRECISION)),
            tf.exp(layer) * tf.constant(alpha, dtype=FLOAT_PRECISION)
        ...
        raise ValueError(
            "activation: Unknown activation type: {!r}".format(activation_type)
        )
